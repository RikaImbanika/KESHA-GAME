...
public class PortalsBase : MonoBehaviour
{
    ...
    public void AddFreePort(string sceneName, string portId)
    {
        ...
        if (!FreePorts[sceneName].Contains(portId))
        {
            if (!TakenFreePorts[sceneName].ContainsKey(portId))
            {
                ...
                Debug.LogError($"I placed freeport {sceneName}, {portId}");
            }
            else
                Debug.LogError($"My freeport {sceneName}, {portId} already placed and already taken.");
        }
        else
            Debug.LogError($"My freeport {sceneName}, {portId} already placed.");
    }
    ...
    public IEnumerator TakeFreePort(string sceneName, string thatPortId, string secSceneName)
    {
        ...
        while (true)
        {
            while (FreePorts[secSceneName].Count <= 0)
            {
                if (S.PortalsBase.TakenFreePorts[sceneName].ContainsKey(thatPortId))
                {
                    ...
                    Debug.LogError($"My freeport {sceneName}, {thatPortId} taken by {secondPortId}!");
                    yield return secondPortId;
                    yield break;
                }

                Debug.LogError($"My freeport {sceneName}, {thatPortId} not taken yet.");
        ...
        Debug.LogError($"I take freeport {secSceneName}, {secondPortId}.");

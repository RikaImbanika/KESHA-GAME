...
public static class S : object
{
	...
	/// ID Merge
	/// </summary>
	public static string IDM(object id1, object id2)
	{
		return $"{id1.ToString()}{id2.ToString()}";
	}

	/// <summary>
	/// ID by scene position, for objects in scene from the beginning
	/// </summary>
	public static string ID(string prefix, GameObject go)
	{
		return $"{go.scene.name}{prefix}{go.transform.position.x:F3}{go.transform.position.y:F3}{go.transform.position.z:F3}";
	}

	/// <summary>
	/// New Unique ID, for new objects
	/// </summary>
	public static string ID(string prefix)
	{
		return S.IDM(prefix, IDs.GetId);
	}
}